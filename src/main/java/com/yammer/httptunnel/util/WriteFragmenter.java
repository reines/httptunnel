 ...
 * {@link ChannelBuffer ChannelBuffers}. If a buffer is bigger than the
 * specified upper bound, the buffer is broken up into two or more smaller
 * pieces.
 * <p>
 * This is utilised by the http tunnel to smooth out the per-byte latency, by
 * placing an upper bound on HTTP request / response body sizes.
 *
	...
	public void writeRequested(ChannelHandlerContext ctx, MessageEvent e) throws Exception {
		...
		if (data.readableBytes() <= splitThreshold)
			...
			for (ChannelBuffer fragment : fragments) {
				final ChannelFuture fragmentFuture = Channels.future(ctx.getChannel(), true);
