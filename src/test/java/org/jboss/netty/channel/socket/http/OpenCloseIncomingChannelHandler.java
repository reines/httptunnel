...
class OpenCloseIncomingChannelHandler extends SimpleChannelHandler {
    ...
    public void assertSatisfied(int timeout) throws InterruptedException {
        ...
        assertTrue("Missed some incoming open events", openLatch.await(timeout, TimeUnit.SECONDS));

        // Wait for the message event
        assertTrue("Missed incoming message received event", messageLatch.await(timeout, TimeUnit.SECONDS));

        // Wait for the close events
        assertTrue("Missed some incoming close events", closeLatch.await(timeout, TimeUnit.SECONDS));
    }
