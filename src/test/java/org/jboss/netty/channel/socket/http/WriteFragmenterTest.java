...
public class WriteFragmenterTest {
	...
	public void testLeavesWritesBeneathThresholdUntouched() {
		...
		ChannelBuffer sentData = NettyTestUtils.checkIsDownstreamMessageEvent(
	...
	public void testSplitsMessagesAboveThreshold_twoChunks() {
		...
		ChannelBuffer chunk1 = NettyTestUtils.checkIsDownstreamMessageEvent(
				downstreamCatcher.events.poll(), ChannelBuffer.class);
		ChannelBuffer chunk2 = NettyTestUtils.checkIsDownstreamMessageEvent(
				downstreamCatcher.events.poll(), ChannelBuffer.class);
	...
	public void testSplitsMessagesAboveThreshold_multipleChunks() {
		...
		for (int i = 0; i < 25; i++) {
			ChannelBuffer chunk = NettyTestUtils.checkIsDownstreamMessageEvent(
					downstreamCatcher.events.poll(), ChannelBuffer.class);
			assertEquals(100, chunk.readableBytes());
		}

		ChannelBuffer endChunk = NettyTestUtils.checkIsDownstreamMessageEvent(
				downstreamCatcher.events.poll(), ChannelBuffer.class);
