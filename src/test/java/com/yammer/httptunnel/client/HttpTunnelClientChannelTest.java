...
import com.yammer.httptunnel.FakeClientSocketChannelFactory;
import com.yammer.httptunnel.FakeSocketChannel;
import com.yammer.httptunnel.UpstreamEventCatcher;
import com.yammer.httptunnel.client.HttpTunnelClientChannel;
...
public class HttpTunnelClientChannelTest {
	...
	/**
	 * used to emulate the selection of a random port in response to a bind
	 * request on an ephemeral port.
	 */
	...
	public static final InetSocketAddress LOCAL_ADDRESS_EPHEMERAL_PORT = InetSocketAddress
	...
	static {
		try {
			...
			RESOLVED_LOCAL_ADDRESS_IPV4 = new InetSocketAddress(localhostIPV4,
					LOCAL_PORT);
			RESOLVED_LOCAL_ADDRESS_IPV6 = new InetSocketAddress(localhostIPV6,
					LOCAL_PORT);
			RESOLVED_LOCAL_ADDRESS_IPV4_EPHEMERAL_PORT = new InetSocketAddress(
					localhostIPV4, 0);
			RESOLVED_LOCAL_ADDRESS_IPV6_EPHEMERAL_PORT = new InetSocketAddress(
					localhostIPV6, 0);
			RESOLVED_LOCAL_ADDRESS_IPV4_SELECTED_PORT = new InetSocketAddress(
					localhostIPV4, OTHER_LOCAL_PORT);
			RESOLVED_LOCAL_ADDRESS_IPV6_SELECTED_PORT = new InetSocketAddress(
					localhostIPV6, OTHER_LOCAL_PORT);
	...
	public void setUp() throws Exception {
		...
		HttpTunnelClientChannelFactory factory = new HttpTunnelClientChannelFactory(
				outboundFactory);
